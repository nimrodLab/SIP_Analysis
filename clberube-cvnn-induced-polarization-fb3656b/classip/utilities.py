...
def xavier_complex(m):
    ...
    gain = 1.0 / torch.tensor(2.0).sqrt()
    if m._get_name().__contains__("Linear"):
        torch.nn.init.xavier_uniform_(m.weight, gain=gain)
        if m.bias:
            torch.nn.init.zeros_(m.bias)
...
class ComplexCrossEntropyLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        real_loss = nn.BCEWithLogitsLoss()(inputs.real, targets.real)
        if torch.is_complex(inputs):
            imag_loss = nn.BCEWithLogitsLoss()(inputs.imag, targets.imag)
...
def train(
    ...
    **scheduler_kwargs,
    ...
    for e in range(n_epoch):
        ...
        verbose_str = f"Epoch: {(e + 1):.0f}, " f"Loss: {history['train'][e]:.4e}"
        ...
        if valid_loader:
            ...
            verbose_str += f", Valid: {history['valid'][e]:.4e}"
