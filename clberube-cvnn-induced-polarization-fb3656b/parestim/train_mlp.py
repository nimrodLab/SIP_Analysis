...
n_epoch = int(1e4)  # nombre de répétitions sur tous les data
...
if __name__ == "__main__":
    ...
    for arc in architectures.keys():
        ...
        for exp in architectures[arc].keys():
            ...
            data_dir = f"./data/{exp.split('-')[0]}"  # relatif à ce .py file
            ...
            X = torch.load(f"{data_dir}/X(n={n_data})-latest.pt", weights_only=True)
            y = torch.load(f"{data_dir}/y(n={n_data})-latest.pt", weights_only=True)
            ...
            for n in range(1, n_repeat + 1):
                ...
                if ((exp == "CVNN") and (arc == "A")) or (
                    (exp == "RVNN") and (arc == "A") and (n < 7)
                ):
                    pass
                    ...
                    dataloader["valid"] = DataLoader(
                        dataset_test, batch_size, shuffle=True
                    )
                    ...
                    torch.save(model.state_dict(), f"{wt_dir}/weights-latest-r{n}.pt")
                    torch.save(losses, f"{res_dir}/losses-latest-r{n}.pt")
                    torch.save(X_test, f"{res_dir}/X_test(n={n_data})-latest-r{n}.pt")
                    torch.save(y_test, f"{res_dir}/y_test(n={n_data})-latest-r{n}.pt")
