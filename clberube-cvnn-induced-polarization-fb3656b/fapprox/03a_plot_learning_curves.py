...
from plotlib import MyScalarFormatter
...
for j, arc in enumerate(["A", "B", "C"]):
    ...
    for i, e in enumerate(experiments):
        ...
        print(arc, e, f"{losses[e].mean(0)[-1]:.3e}")
...
plt.savefig(f"{fig_dir}/fapprox-learning-curves.pdf")
plt.savefig(f"{fig_dir}/fapprox-learning-curves.svg")
plt.savefig(f"{fig_dir}/fapprox-learning-curves.png", dpi=300)
